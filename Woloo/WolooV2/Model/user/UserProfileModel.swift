...
class UserProfileModel: Codable{
    ...
    class FutureSubscription: Codable{
    ...
    static func fetchUserProfileV2(showLoading: Bool? = true ,completion: @escaping (UserProfileModel?) -> Void){
        ...
        if !Connectivity.isConnectedToInternet(){
            completion(nil) // Return nil if no internet
            return
        }
        ...
        let headers = ["x-woloo-token": UserDefaultsManager.fetchAuthenticationToken(),
                       "user-agent": userAgent]
        
        NetworkManager(headers: headers, url: nil, service: .userProfile, method: .get, isJSONRequest: false)
            .executeQuery { (result: Result<BaseResponse<UserProfileModel>, Error>) in
                switch result {
                case .success(let response):
                    print("User Profile API response: ", response)
                    let userData = response.results
                    UserProfileModel.storeUserData(userData)
                    completion(userData)   // 🔥 return updated user data
