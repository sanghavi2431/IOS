...
class UserDefaultsManager {
    ...
    private let recentSearchKey = "recentSearchProducts"
    private let user_selected_address = "user_selected_address"
    private static let DeepLinkType = "deeplink_type"
    private static let ReferralCode = "referral_code"
    ...
    static func storeReferralID(value: String?){
        
        UserDefaults.standard.synchronize()
        UserDefaults.standard.set(value, forKey: ReferralCode)
    }
    
    static func fetchReferralID() -> String{
        UserDefaults.standard.synchronize()
        return UserDefaults.standard.string(forKey: ReferralCode) ?? ""
    ...
    static func clearWahcode() {
        UserDefaults.standard.removeObject(forKey: wahCertificateCode)
        UserDefaults.standard.synchronize()
    }
    
    static func storeDeepLinkType(value: String) {
        
        UserDefaults.standard.set(value, forKey: DeepLinkType)
        UserDefaults.standard.synchronize()
    }
    
    static func fetchDeepLinkType() -> String {
        
        UserDefaults.standard.synchronize()
        return UserDefaults.standard.string(forKey: DeepLinkType) ?? ""
    }
    
    static func clearDeepLinkType() {
        UserDefaults.standard.removeObject(forKey: DeepLinkType)
        UserDefaults.standard.synchronize()
    }
    ...
    func saveRecentSearchesToUserDefaults(_ products: [Products]) {
        let encoder = JSONEncoder()
        if let encoded = try? encoder.encode(products) {
            UserDefaults.standard.set(encoded, forKey: recentSearchKey)
        }
    }

    func getRecentSearchesFromUserDefaults() -> [Products] {
        if let data = UserDefaults.standard.data(forKey: recentSearchKey) {
            let decoder = JSONDecoder()
            if let decoded = try? decoder.decode([Products].self, from: data) {
                return decoded
            }
        }
        return []
    ...
    func saveAddressToUserDefaults(_ address: StoreAddress) {
        do {
            let data = try JSONEncoder().encode(address)
            UserDefaults.standard.set(data, forKey: user_selected_address)
            UserDefaults.standard.synchronize()
            print("Address saved successfully.")
        } catch {
            print("Failed to encode StoreAddress: \(error)")
        }
    }
    
    
    func getAddressFromUserDefaults() -> StoreAddress? {
        if let data = UserDefaults.standard.data(forKey: "user_selected_address") {
            do {
                let address = try JSONDecoder().decode(StoreAddress.self, from: data)
                return address
            } catch {
                print("Failed to decode StoreAddress: \(error)")
            }
        }
        return nil
    }
