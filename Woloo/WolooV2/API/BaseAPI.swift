...
enum APIRestAction : Int {
    ...
    case deletePromotions = 237
    case filterProducts = 238
    case restock_subscriptions = 239
    case check_inventory = 240
}
...
//        parameters.setValue(DEVICE_TYPE, forKey: "platform")
    ...
    func PUTDeleteAction(completion: @escaping (AFDataResponse<Any>) -> Void)
        {
            let urlString = self.urlString();
            
            // print("HEADERS \(headers)")
            print("PUT DELETE \(urlString)")
            print("PARAMETERS \(parameters)")
    //        parameters.setValue(Utility.selectedLanguageCode(), forKey: "lang_code")
    //        parameters.setValue(DEVICE_TYPE, forKey: "platform")
            ...
            AF.request(urlString, method: HTTPMethod.put, parameters: (self.parameters as NSDictionary) as? Parameters, encoding: URLEncoding.default, headers: headers).validate().responseJSON { (response) in
                switch (response.result) {
                case .success:
                    //do json stuff
                    completion(response)
