//
//  HygieneServicesHostRouterAPI.swift
//  Woloo
//
...
class HygieneServicesHostRouterAPI: HygieneBaseAPI {
    ...
    func setupPathComponents(_ action: APIRestAction, strVal1:String, strVal2: String) {
        ...
        if action == .restock_subscriptions{
            subPath = ["\(baseComponent)/restock-subscriptions"]
        }
        
        if action == .check_inventory{
            subPath = ["\(baseComponent)/carts/\(strVal1)/check-inventory"]
        }
