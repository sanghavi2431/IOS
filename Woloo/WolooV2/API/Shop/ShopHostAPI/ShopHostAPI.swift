...
class ShopHostAPI: NSObject{
    ...
    func addAddress(objAddress: StoreAddress?, success: @escaping (_ objCommonWrapper: CustomerCreationWrapper)->Void,
        ...
        parameters.setValue(objAddress?.state ?? "", forKey: "province")
    ...
    func selectShippingMethods(
        strCartId: String?,
        strOptionId: [ShippingOptions]?,
        success: @escaping (_ objCommonWrapper: CreateCart) -> Void,
        failure: @escaping (Error?) -> Void
    ) {
        let parameters = NSMutableDictionary()
        let arrValue = NSMutableArray()

        for objID in strOptionId ?? [] {
            let parametersVal = NSMutableDictionary()
            parametersVal.setValue(objID.id ?? "", forKey: "id")
    ...
    func getOrderSetAPI(strOrderID: String? , success: @escaping (_ objCommonWrapper: CompleteCart)->Void,
                             failure: @escaping (Error?)-> Void){
    ...
    func deletePromoCode(strPromoCodes: [String]?, success: @escaping (_ objCommonWrapper: CreateCart)->Void,
                    failure: @escaping (Error?)-> Void){
        
        let parameters = NSMutableDictionary()
        parameters.setValue(strPromoCodes, forKey: "promo_codes")
        let cartId = UserDefaultsManager.fetchUserCartID()
        
        let api = ShopHostRouterAPI(params: parameters)
       
        api.DELETEAction(action: .deletePromotions, endValue1: "\(cartId)", endValue2: "") { (response) in
    ...
    func getFilterProducts(strCategoryID: String?,optionValue: String?,success: @escaping (_ objCommonWrapper: ProductListWrapper)->Void,
                           failure: @escaping (Error?)-> Void){
        
        let parameters = NSMutableDictionary()
        parameters.setValue("*variants.calculated_price, variants.inventory_quantity,*categories", forKey: "fields")
        parameters.setValue(strCategoryID ?? "", forKey: "category_id")
        if !Utility.isEmpty(optionValue ?? ""){
            parameters.setValue(optionValue ?? "", forKey: "variants[options][value]")
        }
        

        let api = ShopHostRouterAPI(params: parameters)
        
        api.GETAction(action: .filterProducts, endValue1: "", endValue2: "") { (response) in
    ...
    func restockSubscriptionsAPI(strPhone: String?,strVariant_id: String?,success: @escaping (_ objCommonWrapper: ListWrapperRestock<[RestockSubscriptions]>)->Void,
                           failure: @escaping (Error?)-> Void){
        ...
        api.POSTAction(action: .restock_subscriptions, endValue1: "", endValue2: "") { (response) in
            ...
            {
                guard let data = response.data else { return }
                if let objParsed : ListWrapperRestock<[RestockSubscriptions]>? = ListWrapperRestock<[RestockSubscriptions]>.decode(data){
                    success(objParsed!)
                }
            }
        }
    ...
    func checkInventoryAPI(
        success: @escaping (_ objCommonWrapper: CheckInventoryWrapper) -> Void,
        failure: @escaping (_ message: String?, _ errors: CheckInventoryWrapper?) -> Void
    ) {
        
        let parameters = NSMutableDictionary()
        let api = ShopHostRouterAPI(params: parameters)
        let strCartID = UserDefaultsManager.fetchUserCartID()
        api.GETAction(action: .check_inventory, endValue1: "\(strCartID)", endValue2: "") { response in
                if SSError.isErrorReponse(operation: response.response) {
                    // Try decoding CheckInventoryWrapper even on error
                    guard let data = response.data else {
                        failure("No data received", nil)
                        return
                    }
                    if let objParsed = try? JSONDecoder().decode(CheckInventoryWrapper.self, from: data) {
                        failure(objParsed.message, objParsed)
                    } else {
