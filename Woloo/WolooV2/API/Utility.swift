...
//        "-\\x5a\\x53-\\x7f]|\\\\[\\x01-\\x09\\x0b\\x0c\\x0e-\\x7f])+)\\])"
    ...
    class func setCornerRadiusOnPercentage(viewCorner:UIView,percentage: CGFloat) {
        let smallerDimension = min(viewCorner.frame.width, viewCorner.frame.height)
        viewCorner.layer.cornerRadius = (percentage / 100) * smallerDimension
...
//    }
    ...
    static func isValidEmail(_ email: String) -> Bool {
        ...
        static func isEmpty(_ str: String?) -> Bool {
            return str?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ?? true
        }
...
class DecryptAes256CBC {
    ...
    static func decryptAes256CBC(encryptedData: String, secretKey: String) -> String {
        do {
            let parts = encryptedData.components(separatedBy: ":")
            guard parts.count == 2 else { return "Invalid encrypted format" }
            
            let ivHex = parts[0]
            let encryptedHex = parts[1]
            
            guard let ivData = hexStringToData(ivHex),
                  let encryptedData = hexStringToData(encryptedHex),
                  let keyData = secretKey.data(using: .utf8),
                  keyData.count == kCCKeySizeAES256 else {
                return "Invalid key or input data"
            }

            let decryptedData = try aesDecrypt(data: encryptedData, key: keyData, iv: ivData)
            return String(data: decryptedData, encoding: .utf8) ?? "Failed to decode string"
        } catch {
    ...
    private static func aesDecrypt(data: Data, key: Data, iv: Data) throws -> Data {
        ...
        guard cryptStatus == kCCSuccess else {
            throw NSError(domain: "AES", code: Int(cryptStatus), userInfo: nil)
        }
