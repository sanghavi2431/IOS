...
class WolooGuestAPI: NSObject {
    ...
    func updateShopAPI(objProfile:  UserProfileModel.Profile?, success: @escaping (_ objCommonWrapper: BaseResponse<Profile>)->Void,
                     failure: @escaping (Error?)-> Void){
    ...
    func getTermsPage(success: @escaping (_ objCommonWrapper: BaseResponse<TermsModel>)->Void,
                          failure: @escaping (Error?)-> Void){
        ...
        api.GETAction(action: .terms, endValue: "") { (response) in
                    ...
                    {
                        guard let data = response.data else { return }
                        if let objParsed : BaseResponse<TermsModel>? = BaseResponse<TermsModel>.decode(data){
                            success(objParsed!)
                        }
                    }
                }
    ...
    func getAboutPage(success: @escaping (_ objCommonWrapper: BaseResponse<TermsModel>)->Void,
                          failure: @escaping (Error?)-> Void){
        ...
        api.GETAction(action: .about, endValue: "") { (response) in
    ...
    func registerAPI(success: @escaping (_ objCommonWrapper: RegisterWrapper)->Void,
                          failure: @escaping (Error?)-> Void){
        ...
        api.PATCHAction(action: .register, endValue: "") { (response) in
                    ...
                    {
                        guard let data = response.data else { return }
                        if let objParsed : RegisterWrapper? = RegisterWrapper.decode(data){
                            success(objParsed!)
                        }
                    }
                }
    ...
    func creditUserCoins(blogId: Int?,coins: Int?, isGift: Int?, strRemarks: String?,strType: String?,woloo_id: Int?,wolooCoins: Int?,success: @escaping (_ objCommonWrapper: CoinsWrapper)->Void,
                         failure: @escaping (Error?)-> Void){
        
        let parameters = NSMutableDictionary()
        parameters.setValue(blogId, forKey: "blog_id")
        
        parameters.setValue(isGift, forKey: "is_gift")
        parameters.setValue(strRemarks ?? "", forKey: "remarks")
        parameters.setValue(strType ?? "", forKey: "type")
        if woloo_id != 0{
        parameters.setValue(woloo_id ?? "", forKey: "woloo_id")
        parameters.setValue(wolooCoins, forKey: "wolooCoins")
        parameters.setValue(coins, forKey: "coins")
        }
        else{
            parameters.setValue(coins, forKey: "coins")
        }
        ...
        api.POSTAction(action: .creditUserCoins, endValue: "") { (response) in
                    ...
                    {
                        guard let data = response.data else { return }
                        if let objParsed : CoinsWrapper? = CoinsWrapper.decode(data){
                            success(objParsed!)
                        }
                    }
                }
    ...
    func deleteUser(success: @escaping (_ objCommonWrapper: BaseResponse<DeleteUser>)->Void,
                           failure: @escaping (Error?)-> Void){
            ...
            api.PUTDeleteAction(action: .deleteUser, endValue: "\(String(UserDefaultsManager.fetchUserID()))") { (response) in
                        ...
                        {
                            guard let data = response.data else { return }
                            if let objParsed : BaseResponse<DeleteUser>? = BaseResponse<DeleteUser>.decode(data){
                                success(objParsed!)
                            }
                        }
                    }
    ...
    func sendOTP(mobileNumber: String,referral_code: String?,success: @escaping (_ objCommonWrapper: BaseResponse<SendOtpModel>)->Void,
                     failure: @escaping (_ error: String?) -> Void) {
            
            let parameters = NSMutableDictionary()
            parameters.setValue(mobileNumber, forKey: "mobileNumber")
            parameters.setValue(referral_code, forKey: "referral_code")
            
            let api = WolooGuestRouterAPI(params: parameters)
            
            api.POSTAction(action: .sendOtp, endValue: "") { (response) in
                        ...
                        {
                            guard let data = response.data else { return }
                            if let objParsed : BaseResponse<SendOtpModel>? = BaseResponse<SendOtpModel>.decode(data){
                                success(objParsed!)
                            }
                            else{
                                let error = SSError.errorWithData(data: response)
