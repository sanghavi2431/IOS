...
class WolooGuestRouterAPI: BaseAPI {
    ...
    func PUTDeleteAction(action : APIRestAction, endValue:String, completion: @escaping (AFDataResponse<Any>) -> Void) {
            self.setupPathComponents(action, strVal: endValue)
            super.PUTDeleteAction(completion: completion)
        }
    ...
    func setupPathComponents(_ action: APIRestAction, strVal:String) {
        ...
        if action == .creditUserCoins{
            subPath = ["\(baseComponent)/creditUserCoins"]
        }
        
        
        if action == .deleteUser{
                   subPath = ["\(baseComponent)/delete?id=\(strVal)"]
               }
               
 
        
        if action == .sendOtp{
                   subPath = ["\(baseComponent)/sendOTP"]
            }
