...
class WolooHostAPI: NSObject {
                                    ...
                                    failure: @escaping (Error?)-> Void)
        {
            ...
            api.POSTAction(action: .recommendWoloo, endValue: "") { multipartFormData in
                ...
                if SSError.isErrorReponse(operation: response.response) {
                    ...
                    if let errorWrapper = APIErrorResponse.decode(data) {
                        print("API Error:", errorWrapper.message ?? "")
                        failure(NSError(
                            domain: "",
                            code: 0,
                            userInfo: [NSLocalizedDescriptionKey:
                                        errorWrapper.message ?? "Something went wrong"]
                        ))
                    } else {
                        failure(nil)
                    }
    ...
    func searchWolooHost(strQuery: String?,page: Int?,lat: Double?, long: Double?,success: @escaping (_ objCommonWrapper: BaseResponse<SearchListWrapper>)->Void,
                         failure: @escaping (Error?)-> Void){
        ...
        locationDict.setValue(lat ?? 0.0, forKey: "lat")
        locationDict.setValue(long ?? 0.0, forKey: "lng")
    ...
    func powderRoomPaymentAPI(powderRoomID: Int?, amount: Int?,success: @escaping (_ objCommonWrapper: BaseResponse<PowderRoomPass>)->Void,
                              failure: @escaping (Error?)-> Void){
        
        
        let parameters = NSMutableDictionary()
        parameters.setValue(powderRoomID ?? "", forKey: "powder_room_id")
        parameters.setValue(amount ?? "", forKey: "amount")
        
        let api = WolooHostRouterAPI(params: parameters)
        
        api.POSTAction(action: .powderRoomPayment, endValue: "") { (response) in
                    ...
                    {
                        guard let data = response.data else { return }
                        if let objParsed : BaseResponse<PowderRoomPass>? = BaseResponse<PowderRoomPass>.decode(data){
                            success(objParsed!)
                        }
                    }
                }
