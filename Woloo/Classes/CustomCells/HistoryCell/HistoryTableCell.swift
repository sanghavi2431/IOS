...
class HistoryTableCell: UITableViewCell {
    ...
    @IBOutlet weak var vwBackLblDistance: UIView!
    ...
    override func awakeFromNib() {
        ...
        self.vwBackCibil.cornerRadius = 10.0
    }
...
//        self.vwBackCibil.isHidden = true
        self.vwBackLblDistance.isHidden = true
        
        lblWolooName.text = detail.name ?? ""
        lblWolooAddress.text = detail.address ?? ""
        self.lblTime.text = "2.3 Hrs"
        self.lbldistance.text = "5 kms"
        self.vwBackCibil.backgroundColor = UIColor(hexString:  detail.cibil_score_colour ?? "#00FF38")
        
        self.lblCibilScore.text =  detail.cibil_score
        
        if let images = detail.image , images.count > 0 {
            let imageUrl = images.first
            if imageUrl?.count ?? 0 > 0 {
                let url = "\(UserDefaultsManager.fetchUserData()?.profile?.baseUrl ?? "")\(imageUrl ?? "")"
    ...
    func configureBookmarkTableCell(objNearBy: NearbyResultsModel?){
        ...
        self.vwBackCibil.backgroundColor = UIColor(hexString:  self.objNearBy.cibil_score_colour ?? "#00FF38")
