...
class EditProfileViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        // self.setupUI()
        lblHeadertitle.font = UIFont(name: "CenturyGothic-Bold", size: 18)
        lblHeaderDesc.font = UIFont(name: "CenturyGothic", size: 12)
        self.setupUIV2()
        //        }
    }
...
//        self.userDataV2 = UserProfileModel()
//        self.userDataV2?.profile =  UserProfileModel.Profile()
...
extension EditProfileViewController {
    ...
    func editProfileV2(){
        guard let name = nameTextField.text, !Utility.isEmpty(name) else {
            self.showToast(message: "Please enter Name.")
            return
        }
        guard let email = emailTextField.text, !Utility.isEmpty(email) else {
            self.showToast(message: "Please enter Email.")
            return
        }
        guard let city = cityTextField.text, !Utility.isEmpty(city) else {
            self.showToast(message: "Please enter City.")
            return
        }
        guard let pincode = pincodeTextField.text, !Utility.isEmpty(pincode) else {
            self.showToast(message: "Please enter Pincode.")
            return
        }
        guard let address = addressTextField.text, !Utility.isEmpty(address) else {
            self.showToast(message: "Please enter Address.")
            return
        }
...
extension EditProfileViewController : SearchLocationDelegate {
    func onCitySelected(cityName: String) {
        ...
        self.userDataV2?.profile?.city = cityName
