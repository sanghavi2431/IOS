...
extension EditProfileViewController: UITableViewDelegate, UITableViewDataSource, GenderSelectionCellDelegate, EditProfileTxtCellProtocol{
    ...
    func didTextFieldChanged(objUserProfileModel: UserProfileModel?) {
        ...
        self.userDataV2?.profile = objUserProfileModel?.profile ?? UserProfileModel.Profile()
        
        self.userDataV2?.profile?.name = objUserProfileModel?.profile?.name ?? ""
        self.userDataV2?.profile?.email = objUserProfileModel?.profile?.email ?? ""
        self.userDataV2?.profile?.city = objUserProfileModel?.profile?.city ?? ""
        self.userDataV2?.profile?.pincode = objUserProfileModel?.profile?.pincode ?? ""
        self.userDataV2?.profile?.dob = objUserProfileModel?.profile?.dob ?? ""
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if indexPath.section == 0{
            ...
            if indexPath.row == 5{
                ...
                print("name",self.userDataV2?.profile?.name ?? "")
                print("email",self.userDataV2?.profile?.email ?? "")
                print("city",self.userDataV2?.profile?.city ?? "")
                print("pincode",self.userDataV2?.profile?.pincode ?? "")
                print("address",self.userDataV2?.profile?.address ?? "")
                
                guard let name = self.userDataV2?.profile?.name, !Utility.isEmpty(name) else {
                    self.showToast(message: "Please enter Name.")
                    return
                }
                guard let email = self.userDataV2?.profile?.email, !Utility.isEmpty(email) else {
                    self.showToast(message: "Please enter Email.")
                    return
                }
                guard let city = self.userDataV2?.profile?.city, !Utility.isEmpty(city) else {
                    self.showToast(message: "Please enter City.")
                    return
                }
                guard let pincode = self.userDataV2?.profile?.pincode, !Utility.isEmpty(pincode) else {
                    self.showToast(message: "Please enter Pincode.")
                    return
                }
//                guard let address = self.userDataV2?.profile?.address, !Utility.isEmpty(address) else {
