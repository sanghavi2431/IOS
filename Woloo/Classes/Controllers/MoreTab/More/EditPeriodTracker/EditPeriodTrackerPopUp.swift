...
class EditPeriodTrackerPopUp: UIViewController, UITextFieldDelegate {
    ...
    func setupUI() {
        // if selectedDate is nil → use current date
        let dateToUse = selectedDate ?? Date()
        selectedDate = dateToUse  // update your stored property too

        dayTextField.text = dateToUse.convertDateToString("dd")
        monthTextField.text = dateToUse.convertDateToString("MMM")
        yearTextField.text = dateToUse.convertDateToString("yyyy")
        ...
        // set date picker's current date
    ...
    func validatePeriodTrackingInputs(selectedDate: Date?, periodLength: Int?, cycleLength: Int?) -> (isValid: Bool, errorMessage: String?) {
            ...
            if let periodLength = periodLength, let cycleLength = cycleLength {
                if periodLength < 2 || periodLength > 8 {
                    return (false, "Bleeding Days should be between 2-8 days")
