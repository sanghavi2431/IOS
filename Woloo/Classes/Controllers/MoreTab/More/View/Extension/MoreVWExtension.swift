...
extension MoreVC: UITableViewDelegate, UITableViewDataSource, DashboardViewModelDelegate, MoreViewModelDelegate, MoreGridTableCellProtocol, LogOutBtnCellProtocol, EditThirstReminderPopUpDelegate, SetThirstReminderPopUpDelegate, EditCycleViewModelDelegate, EditPeriodTrackerPopUpDelegate, LogOutPopUpVCDelegate, WolooAlertPopUpViewDelegate, BuySubscriptionDelegate, UserProfileTableViewCellDelegate{
    ...
    func didTapSubStatus() {
        ...
        viewController.isMyPlan = true
        self.navigationController?.pushViewController(viewController, animated: true)
    }
    
    
    //MARK: - BuySubscriptionDelegate
    func didPurchasedSubscription() {
        self.isBuySubscription = true
        self.objMoreViewModel.creditUserCoins(blogId: 0, coins: Int(UserDefaultsManager.fetchAppConfigData()?.woloo_subscription_points ?? "0"), isGift: 0, strRemarks: "Buy Subscription", strType: "Manual Credit", woloo_id: 0, wolooCoins: 0)
        
        Global.addNetcoreEvent(eventname: self.netcoreEvents.wolooSubscriptionPoints, param: ["platform": "iOS"])
    ...
    func closePopUp() {
        ...
        self.objMoreViewModel.creditUserCoins(blogId: 0, coins: Int(UserDefaultsManager.fetchAppConfigData()?.set_thirst_reminder ?? "0"), isGift: 0, strRemarks: "Save Thirst Reminder", strType: "Manual Credit", woloo_id: 0, wolooCoins: 0)
    ...
    func didReceiveCreditUserCoinsResponse(objResponse: CoinsWrapper) {
        ...
        if self.isBuySubscription == true{
            DispatchQueue.main.async{
                let objController = BlogsPointsPopUpVC(nibName: "BlogsPointsPopUpVC", bundle: nil)
               // objController.delegate = self
                objController.pointCount = UserDefaultsManager.fetchAppConfigData()?.woloo_subscription_points ?? "0"
...
//                        Smartech.sharedInstance().trackEvent("User_logout_success", andPayload: ["":""])
    ...
    func didSelectMinutes(minutes: String?) {
        // Validate that hours don't exceed 12
        guard let hours = Int(minutes ?? ""), hours > 0 else {
                self.showToast(message: "Please enter valid hours (greater than 0)")
                return
            }
            ...
            self.setFrequency = "\(hours)" // normalize input
            self.thirstReminderYesV2()
        
        Global.addNetcoreEvent(eventname: self.netcoreEvents.setThirstReminder, param: ["is_thirst_reminder": "1", "thirst_reminder_hours": minutes ?? "","platform": "iOS"])
    }
...
//        }
        if indexPath.section == 0{
            ...
            cell?.configureUserProfileTableViewCell(objUser: self.objUser)
...
//                }
    ...
    //MARK: - Schedule Thirst Reminder Notification
    func scheduleThirstReminderNotification(frequency: String) {
        // Request notification permission if not already granted
        let center = UNUserNotificationCenter.current()
        center.getNotificationSettings { (settings) in
            if settings.authorizationStatus == .authorized {
                // Get all pending notifications and remove thirst reminder ones
                center.getPendingNotificationRequests { (requests) in
                    let thirstReminderIds = requests.filter { $0.identifier.hasPrefix("ThirstReminder") }.map { $0.identifier }
                    if !thirstReminderIds.isEmpty {
                        center.removePendingNotificationRequests(withIdentifiers: thirstReminderIds)
                        print("Removed \(thirstReminderIds.count) existing thirst reminder notification(s)")
                    }
                    
                    DispatchQueue.main.async {
                        // Create notification content
                        let content = UNMutableNotificationContent()
                        content.title = "Thirst reminder"
                        content.body = "It's time to drink water!"
                        content.sound = UNNotificationSound.default
                        content.badge = 1
                        
                        // Calculate time interval in seconds (frequency is in hours)
                        let setFrequencyTime = 60 * 60 * (Int(frequency) ?? 1)
                        print("Thirst reminder frequency time set: \(setFrequencyTime) seconds (\(frequency) hours)")
                        
                        // Create trigger that repeats every setFrequencyTime seconds
                        let trigger = UNTimeIntervalNotificationTrigger(timeInterval: TimeInterval(setFrequencyTime), repeats: true)
                        
                        // Create unique identifier with frequency
                        let identifier = "ThirstReminder_\(frequency)"
                        ...
                        center.add(request) { (error) in
                            if let error = error {
                                print("Error scheduling thirst reminder notification: \(error.localizedDescription)")
                            } else {
                                print("Thirst reminder notification scheduled successfully with identifier: \(identifier), repeating every \(setFrequencyTime) seconds")
                            }
                        }
                    }
                }
                ...
                center.requestAuthorization(options: [.alert, .sound, .badge]) { (granted, error) in
                    if granted {
                        DispatchQueue.main.async {
                            self.scheduleThirstReminderNotification(frequency: frequency)
                        }
                    } else {
        ...
        NetworkManager(data: param, headers: headers, url: nil, service: .thirstReminder, method: .post, isJSONRequest: true).executeQuery { (result: Result<BaseResponse<ThirstReminderModel>, Error>) in
            ...
            case .success(let response):
                ...
                DispatchQueue.main.async {
                    ...
                    // Schedule Local Notification after popup is shown
                    self.scheduleThirstReminderNotification(frequency: setFrequency ?? "")
                }
