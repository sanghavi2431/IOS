...
class PeriodTrackerViewController: UIViewController, DailyLogViewControllerDelegate, EditPeriodTrackerPopUpDelegate, EditCycleViewModelDelegate{
...
//        }
    ...
    func didReceivePeriodTrackerResponse(objResponse: BaseResponse<ViewPeriodTrackerModel>) {
        ...
        Global.addNetcoreEvent(eventname: self.netCoreEvents.setPeriodTracker, param: ["period_date":"\(objResponse.results.periodDate ?? "")","cycle_length":"\(objResponse.results.cycleLength ?? 0)","period_length":"\(objResponse.results.periodLength ?? 0)","luteal_length":"\(objResponse.results.lutealLength ?? 0)","menstrual_length":"\(objResponse.results.periodLength ?? 0)", "user_id":"\(UserModel.user?.userId ?? 0)", "platform":"iOS"])
        
        DispatchQueue.main.asyncAfter(deadline: .now() + seconds) {
            // Put your code which should be executed with a delay here
            Global.hideIndicator()
            print("objResponse cycle length Set PT:", objResponse.results.cycleLength ?? "")

            self.objPeriodTrackerViewModel.creditUserCoins(blogId: 0, coins: Int(UserDefaultsManager.fetchAppConfigData()?.set_period_tracker ?? "0"), isGift: 0, strRemarks: "Period Tracker", strType: "Manual Credit", woloo_id: 0, wolooCoins: 0)
            
            self.getPeriodTrackerAPIV2()
    ...
    var objViewPeriodTracker = ViewPeriodTrackerModel()
...
//        self.objPeriodTrackerViewModel.creditUserCoins(blogId: 0, coins: 10, isGift: 0, strRemarks: "Period Tracker", strType: "Manual Credit")
    }
    ...
    private func setupUI() {
        ...
        self.lblNavigationTitle.text = "Period Tracker"
    }
...
extension PeriodTrackerViewController: UITableViewDelegate, UITableViewDataSource, TrackerCalenderCellDelegate {
    
    
    
    //MARK: - TrackerCalenderCellDelegate
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case .circularView:
            ...
            cell.infoHandlerEdit = { [weak self] in
                guard let weak = self else { return }
                print("Edit Period Info")
                let objController = EditPeriodTrackerPopUp.init(nibName: "EditPeriodTrackerPopUp", bundle: nil)
                objController.delegate = self
                objController.selectedDate = self?.objViewPeriodTracker.periodDate?.toDate()
                objController.periodLength = self?.objViewPeriodTracker.periodLength ?? 0
                objController.cycleLength = self?.objViewPeriodTracker.cycleLength ?? 0
            ...
            //cell.setDays()
            cell.setDaysV2(objViewPeriodTracker: self.objViewPeriodTracker)
            return cell
        case .calenders:
            ...
            cell.delegate = self
            cell.allMonthtrackerInfoV2 = self.allTrackerInfov2
            cell.configurefillListOfCyclesv2(objViewPeriodTrackerModel: self.allTrackerInfov2)
            cell.dateTitleLabel.text = cell.calenderView.currentPage.convertDateToString("MMMM yyyy")
            ...
            NetworkManager(headers: headers, url: nil, service: .viewperiodtracker, method: .get, isJSONRequest: false).executeQuery {(result: Result<BaseResponse<ViewPeriodTrackerModel>, Error>) in
                ...
                case .success(let response):
                    ...
                    DispatchQueue.main.async{
                        ...
                        print("objResponse cycle length View PT:", response.results.cycleLength ?? "")
                        self.objViewPeriodTracker = response.results
                        self.trackerTableView.reloadData()
                    }
