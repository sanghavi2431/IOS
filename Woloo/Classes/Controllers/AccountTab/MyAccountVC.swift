...
//            }
            cell.imgVWPiggyBank.image = UIImage.gifFromAsset(named: "PiggyBankAnimation")
            cell.lblWolooPoints.text = "\(self.objUserCoinModel.totalCoins ?? 0) points"
//            var giftCoins = 0
//            if userCoins.giftCoins != nil {
//                giftCoins = userCoins.giftCoins ?? 0
//            }
            cell.lblGiftPoints.text = "\u{20B9} \(self.objUserCoinModel.totalGiftCoins ?? 0)"
...
//
    ...
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        if indexPath.section == 2 && indexPath.row == coinHistoryV2.count - 1 { // last cell
            if (coinHistoryModelV2.total_count ?? 0) > coinHistoryV2.count {
                getCoinHistoryV2(param: ["pageIndex": pageNumber])
            } else {
                isLastPage = true
            }
...
extension MyAccountVC {
    ...
    func getCoinHistoryV2(param: [String: Any]) {
        guard !isFetching, !isLastPage else { return } // stop duplicate requests
        isFetching = true
        ...
        if !Connectivity.isConnectedToInternet() {
            ...
            showAlertWithActionOkandCancel(
                Title: "Network Issue",
        ...
        NetworkManager(data: data, headers: headers, url: nil, service: .coinHistory, method: .get, isJSONRequest: false)
            .executeQuery { (result: Result<BaseResponse<CoinHistoryModelV2>, Error>) in
                ...
                case .success(let response):
                    ...
                    DispatchQueue.main.async {
                        ...
                        let newData = response.results.history ?? []
                        
                        if newData.isEmpty {
                            self.isLastPage = true
                        } else {
                            if self.pageNumber == 1 {
                                self.coinHistoryV2 = newData
                            } else {
                                self.coinHistoryV2.append(contentsOf: newData)
                            }
                            self.pageNumber += 1
                            self.tableView.reloadSections(IndexSet(integer: 2), with: .automatic)
                        }
