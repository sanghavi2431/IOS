...
protocol BuySubscriptionDelegate: NSObjectProtocol{
    func didPurchasedSubscription()
}
...
class BuySubscriptionVC: UIViewController, MFMailComposeViewControllerDelegate {
    ...
    var objMySubscriptionModel = MySubscriptionModel()
    
    var objMoreViewModel = MoreViewModel()
    weak var delegate: BuySubscriptionDelegate?
    ...
    override func viewDidLoad() {
       ...
       // self.titleLabel.text = isMyPlan ? "My Subscription" : "Woloo Premium"
        ...
        cancelReasonArray = UserDefaultsManager.fetchAppConfigData()?.CUSTOM_MESSAGE?.cancelSubscriptionReasons?.components(separatedBy: ",") ?? ["No reason to cancel"]
        ...
        if isMyPlan {
            ...
            //getMySubscription()
            getMySubscriptionV2()
            self.lblScreenTitle.text = "My Subscription"
          //  getMoreProfile(false)
           
        } else {
            //            InAppPurchase.shared.refreshReceipt()
            //getPlans()
            self.lblScreenTitle.text = "Pee'er Club Membership"
            getPlansV2()
           // vwCancelSubscription.isHidden = true
...
//        DELEGATE.rootVC?.tabBarVc?.showFloatingButton()
    ...
    @IBAction func okayPaymentSuccessAction(_ sender: UIButton) {
        if self.delegate != nil {
            self.delegate?.didPurchasedSubscription()
        }
...
//            self.paymentSuccessAPIV2(payment_id: "", plan_id: dict.plan_id ?? "", order_id: "", future: btnRenewLater.isSelected ? true : false, userGiftPoints: true)
            ...
            self.paymentSuccessGiftVoucher(plan_id: dict.plan_id ?? "", future: btnRenewLater.isSelected ? 1 : 0, userGiftPoints: true)
...
//        print("pay through razorpay")
    ...
    @IBAction func clickedDeleteUser(_ sender: UIButton) {
        DispatchQueue.main.async {
            let objController = DeleteUserPopUpVC.init(nibName: "DeleteUserPopUpVC", bundle: nil)
...
extension BuySubscriptionVC : UITableViewDelegate, UITableViewDataSource, DeleteUserPopUpVCDelegate, MoreViewModelDelegate{
    ...
    func didReceiceCreditUserCoinsError(strError: String) {
        ...
        func didReceiveDeleteResponse(objCommonWrapper: BaseResponse<DeleteUser>) {
            ...
            Global.hideIndicator()
                self.showToast(message: objCommonWrapper.results.Response ?? "")
    ...
    //MARK: - DeleteUserPopUpVCDelegate
    func didClickedYes() {
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
            // Code to execute after 0.5 seconds delay
            Global.showIndicator()
            self.objMoreViewModel.deleteUserAPI()
        }
...
//        return cell
       ...
       //MySubscriptionTableCell
        ...
        cell?.configureMySubscriptionTableCell(objSubscriptiondate: getAllPlans[indexPath.row], isPlan: self.isMyPlan)
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        print("self.objMySubscriptionModel.futureSubscription?.count", self.objMySubscriptionModel.futureSubscription?.count ?? 0)
        
        if let expiryStr = UserDefaultsManager.fetchUserData()?.profile?.expiry_date,
           let expiryDateUTC = expiryStr.toISODate() {

            let calendar = Calendar.current

            let expiryDate = calendar.startOfDay(for: expiryDateUTC)
            ...
            if days < 0 {
                ...
                bottomPaymentBtn.isHidden = false
                vwRenewSubscription.isHidden = true
                bottomPaymentView.isHidden = false
                bottomContainerPaymentView.isHidden = false
                tableView.reloadData()
            }
            else if self.objMySubscriptionModel.futureSubscription?.count ?? 0 == 0{
                selectedIndex = indexPath.row
                ...
                objController.isComeFrom = "futureSubscription"
...
//                print("Get plans parsed value: \(self.getAllPlans)")
    ...
    func paymentSuccessAPIV2(payment_id: String?, plan_id: String?, order_id: String?, future: Bool, userGiftPoints: Bool?, paymentSignature: String?){
        ...
        let data = ["payment_id": payment_id ?? "", "plan_id": plan_id ?? "", "order_id": order_id ?? "", "future": future, "userGiftPoints": userGiftPoints ?? false, "payment_signature": paymentSignature ?? ""] as [String : Any]
    ...
    func paymentSuccessGiftVoucher(plan_id: String?, future: Int?, userGiftPoints: Bool?){
        ...
        NetworkManager(data: data, headers: headers, url: nil, service: .submitSubscription, method: .post, isJSONRequest: true).executeQuery { (result: Result<BaseResponse<SubmitSubscriptionPurchaseModel>, Error>) in
            ...
            case .failure(let error):
                print("Request Submission Error", error)
                DispatchQueue.main.async {
                    self.showToast(message: "No sufficient points !")
                }
            }
        }
    ...
    func getMySubscriptionV2(_ showLoading: Bool? = true){
        ...
        NetworkManager(headers: headers, url: nil, service: .mySubscription, method: .get, isJSONRequest: false).executeQuery { (result: Result<BaseResponse<MySubscriptionModel>, Error>) in
            ...
            case.success(let response):
                ...
                self.objMySubscriptionModel = response.results
                ...
                if self.isMyPlan{
                    self.getAllPlans.append(response.results.activeSubscription!)
                    
                    if response.results.futureSubscription?.count ?? 0 > 0{
                        self.getAllPlans.append(contentsOf: response.results.futureSubscription!)
                    }
                }
                
                for i in 0..<self.getAllPlans.count {
                    if self.getAllPlans[i].plan_id == response.results.activeSubscription?.plan_id {
                        print("active subscription matched")
                        self.getAllPlans[i].is_active = true
                    }
                }
                
                if response.results.futureSubscription?.count ?? 0 > 0 {
                    for i in 0..<self.getAllPlans.count {
                        if self.getAllPlans[i].plan_id == response.results.futureSubscription?[0].plan_id {
                            print("future subscription matched")
                            self.getAllPlans[i].is_future = true
                            self.futurePlanSubscriptionV2?.append(self.getAllPlans[i])
    ...
    func configuredMailComposeViewController() -> MFMailComposeViewController {
        ...
        let supportEmailUrl = "woloo.in@gmail.com"
...
extension BuySubscriptionVC: RazorpayPaymentCompletionProtocolWithData {
    ...
    func onPaymentError(_ code: Int32, description str: String, andData response: [AnyHashable : Any]?) {
        print("error: ", code)
        self.showToast(message: "Payment Failed..!!")
        //        DELEGATE.rootVC?.tabBarVc?.showTabBar()
    }
    ...
    func onPaymentSuccess(_ payment_id: String, andData response: [AnyHashable : Any]?) {
     ...
     //   self.paymentSuccessAPIV2(payment_id: payment_id , plan_id: planIdSelected ?? "", order_id: subscriptionId, future: btnRenewLater.isSelected ? true : false, userGiftPoints: giftVoucherBtn.isSelected ? true : false, paymentSignature: response?["razorpay_signature"] as? String ?? "")
