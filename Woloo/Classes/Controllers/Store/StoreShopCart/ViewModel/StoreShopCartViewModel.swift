...
struct StoreShopCartViewModel{
    ...
    func selectShippingMethodsAPI(strCartId: String?,strOptionId: [ShippingOptions]?){
        
        ShopHostAPI().selectShippingMethods(strCartId: strCartId ?? "", strOptionId: strOptionId ?? [ShippingOptions]()) { objCommonWrapper in
    ...
    func deletePromotionCode(strPromoCodes: [String]?){
        ShopHostAPI().deletePromoCode(strPromoCodes: strPromoCodes) { objCommonWrapper in
            self.delegate?.didReceievDeletePromoCodeAPISuccess(objResponse: objCommonWrapper)
        } failure: { error in
            self.delegate?.didReceievDeletePromoCodeAPIError(strError: error?.localizedDescription ?? "")
        }

    }
    
    func checkInventoryAPICall(){
        ShopHostAPI().checkInventoryAPI { objCommonWrapper in
            self.delegate?.didReceievCheckInventoryAPISuccess(objResponse: objCommonWrapper)
        } failure: { message, errors in
            self.delegate?.didReceievCheckInventoryAPIError(strError: errors ?? CheckInventoryWrapper())
        }
