...
//        objController.objAddress = self.objCustomerInfo.addresses ??  [StoreAddress]()
    ...
    func didReceievCustomerInfoAPISuccess(objResponse: CustomerCreationWrapper) {
        ...
        self.listAddress = self.objCustomerInfo.addresses ??  [StoreAddress]()
    ...
    //MARK: - CheckInventoryPopUpVCDelegate
    func didClickedBtnNotify(objCartItems: CartItems) {
        self.objStoreHomePageViewModel.restockSubscriptionAPI(strPhone: UserDefaultsManager.fetchUserMob(), strVariant: objCartItems.variant_id ?? "")
    ...
    func closePopUp() {
        ...
        if self.isComeFrom == "OrderPlaced"{
            self.objStoreShopCartViewModel.ecomCoinUpdateAPI(strOrderID: orderID)
            print("complete order", self.orderID)
            let objController = StoreOrderDetailsVC.init(nibName: "StoreOrderDetailsVC", bundle: nil)
            objController.orderId = orderID
            objController.objStoreItems = [self.objCompleteCart]
            UserDefaultsManager.storeOrderID(value: self.orderID)
    ...
    func didDeletePromoCode(strPromocode: String?) {
        print("Call delete promocode api")
        Global.showIndicator()
        self.objStoreShopCartViewModel.deletePromotionCode(strPromoCodes: [strPromocode ?? ""])
    }
...
//        
    ...
    //check inventory API
    func didReceievCheckInventoryAPISuccess(objResponse: CheckInventoryWrapper) {
        self.objCheckInventory = objResponse
    ...
    func didReceievCheckInventoryAPIError(strError: CheckInventoryWrapper) {
        Global.hideIndicator()
        self.objCheckInventory = strError
        print("inventory items count", self.objCheckInventory.errors?.count ?? 0)
        
        
        var matchedCartItems: [CartItems] = []

           for listCheckInv in self.objCheckInventory.errors ?? [] {
               for listCart in self.objCartItems.items ?? [] {
                   if listCheckInv.variant_id == listCart.variant_id {
                       matchedCartItems.append(listCart)
                       break // optional: stop after first match
                   }
               }
           }
        let objController = CheckInventoryPopUpVC(nibName: "CheckInventoryPopUpVC", bundle: nil)
        objController.delegate = self
        objController.listItems = self.objCheckInventory.errors ?? [Errors]()
        objController.listCartItems = matchedCartItems
        let popup = STPopupController(rootViewController: objController)
        popup.style = .bottomSheet
        popup.present(in: DELEGATE.window?.rootViewController ?? self)
    ...
    func didReceievGetCartListAPISuccess(objResponse: CreateCart) {
        ...
        if self.objCartItems.items?.count ?? 0 > 0{
            ...
            self.lblNoItemAdded.isHidden = false
            self.tableView.isHidden = true
            self.vwBottom.isHidden = true
        }
        
        if self.objCartItems.promotions?.count ?? 0 > 1{
            self.objPromotions = self.objCartItems.promotions?[1] ?? Promotions()
        }
    ...
    func didRecieveGetShippingOptionsAPISuccess(objResponse: ShippingOptionsWrapper) {
        ...
        if self.objShippingOptions.count > 0{
            ...
            self.objStoreShopCartViewModel.selectShippingMethodsAPI(strCartId: UserDefaultsManager.fetchUserCartID(), strOptionId: self.objShippingOptions)
        }
    ...
    func didRecieveApplyPromotionsAPI(objWrapper: CreateCart) {
        Global.hideIndicator()
        self.objCartItems.promotions = objWrapper.cart?.promotions ?? [Promotions]()
        
        self.objCartItems.discount_total = objWrapper.cart?.discount_total ?? 0
        self.objCartItems.original_item_total = objWrapper.cart?.original_item_total ?? 0
        self.objCartItems.total = objWrapper.cart?.total ?? 0
        self.objCartItems.shipping_total = objWrapper.cart?.shipping_total ?? 0
        
        DispatchQueue.main.async {

            let objController = WolooAlertPopUpView.init(nibName: "WolooAlertPopUpView", bundle: nil)
            self.isComeFrom = "CoinsApplied"
    ...
    func didRecieveCompleteCartAPISuccess(objResponse: CompleteCart) {
        print("complete order", objResponse.order_set.id ?? "")
        self.orderID = objResponse.order_set.id ?? ""
        self.objCompleteCart = objResponse.order_set
        UserDefaultsManager.storeOrderID(value: objResponse.order_set.id ?? "")
    ...
    func didRecieveCompleteCartAPIError(strError: String) {
        ...
        DispatchQueue.main.async {
            ...
            let objController = WolooAlertPopUpView.init(nibName: "WolooAlertPopUpView", bundle: nil)
            objController.delegate = self
            self.isComeFrom = "OrderNotPlaced"
            objController.isComeFrom = "OrderNotPlaced"
    ...
    func didReceievDeletePromoCodeAPISuccess(objResponse: CreateCart) {
        Global.hideIndicator()
        self.objCartItems.promotions = objResponse.cart?.promotions ?? [Promotions]()
        
        self.objCartItems.discount_total = objResponse.cart?.discount_total ?? 0
        self.objCartItems.original_item_total = objResponse.cart?.original_item_total ?? 0
        self.objCartItems.total = objResponse.cart?.total ?? 0
        self.objCartItems.shipping_total = objResponse.cart?.shipping_total ?? 0
        
        self.tableView.reloadData()
    }
    
    func didReceievDeletePromoCodeAPIError(strError: String) {
        Global.hideIndicator()
        self.showToast(message: strError)
    }
    ...
    func didClickedPayViaBtn() {
        ...
        if Utility.isEmpty(self.objSelectedAddress.address_1 ?? ""){
            DispatchQueue.main.async {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0{
            ...
            if indexPath.row == 0{
                ...
                cell?.configureCartItemCount(count: self.objCartItems.items?.count ?? 0)
                cell?.selectionStyle = UITableViewCell.SelectionStyle.none
                return cell!
            }
            ...
            if indexPath.row == 0{
                ...
                if self.objCartItems.promotions?.count ?? 0 > 0{
                    cell?.configureCartPromoCodeCell(objCoins: self.objUserCoinModel, objPromotion:  self.objCartItems.promotions?[0] ?? Promotions())
                }
                else{
                    cell?.configureCartPromoCodeCell(objCoins: self.objUserCoinModel, objPromotion: Promotions())
                }
                ...
                cell?.delegate = self
                cell?.configureCartPromoCodeCell(objPromotion: self.objPromotions)
                cell?.selectionStyle = UITableViewCell.SelectionStyle.none
                return cell!
            }
