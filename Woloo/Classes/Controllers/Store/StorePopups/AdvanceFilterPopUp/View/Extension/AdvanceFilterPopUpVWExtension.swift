    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.row == 0{
           ...
           // cell?.setDataV2(tags: wolooStoreDOV2?.getAllOfferNameV2 ?? [])
            ...
            //size cell
            var cell: FilterOptionTableCell? = tableView.dequeueReusableCell(withIdentifier: "FilterOptionTableCell") as! FilterOptionTableCell?
            
            if cell == nil {
                cell = (Bundle.main.loadNibNamed("FilterOptionTableCell", owner: self, options: nil)?.last as? FilterOptionTableCell)
            }
          
            cell?.configureFilterOptionTableCell(strOption: self.filteredOptionsValues) { updatedOptions in
                self.filteredOptionsValues = updatedOptions
            }
