...
extension StoreItemDetailsVC: UITableViewDelegate, UITableViewDataSource, ShopCartPopUpVCDelegate, ShopCheckoutPopUpVCDelegate, OrderSummaryPopUpVCDelegate, ProductRatingCartCellDelegate, StoreItemDetailViewModelDelegate, ProductItemInfoCellDelegate, ProductItemImageCellDelegate, ProductItemColorCellProtocol, ProductItemRecentSearchCellDelegate{
    
    //MARK: - ProductItemRecentSearchCellDelegate
    ...
    func didSelectItem(objProductOptionValue: ProductOptionValue?) {
        guard let selectedValue = objProductOptionValue else { return }

        // Loop through all option groups (e.g., color, size, etc.)
        for opt in self.objProduct.options ?? [] {
            // Check if the selected value belongs to this option group
            if opt.values?.contains(where: { $0.id == selectedValue.id }) == true {
                // Mark only this group for update
                for val in opt.values ?? [] {
                    val.isSelected = (val.id == selectedValue.id)
                }

                // Store selected value
                if let optionId = opt.id, let value = selectedValue.value {
                    selectedOptions[optionId] = value
                }

                break // Stop here! Don’t touch other option groups
            }
        }

        // Reload UI and try to match variant
        self.tableView.reloadData()

        if let variant = findMatchingVariant() {
            print("Selected variant: \(variant.title ?? "") with ID: \(variant.id ?? "")")
            self.lblVariant.text = variant.title ?? ""
            self.lblPrice.text = String(format: "%@%d/-","\u{20B9}",variant.calculated_price?.calculated_amount ?? 0)
            
            let originalAmount = variant.calculated_price?.original_amount ?? 0
            ...
            self.objProduct.variants?[0].id  = variant.id ?? ""
        }
    }

    
    func findMatchingVariant() -> ProductVariants? {
        guard !selectedOptions.isEmpty else { return nil }

        for variant in self.objProduct.variants ?? [] {
            guard let variantOptions = variant.options else { continue }

            var matchCount = 0

            for vo in variantOptions {
                if let selectedValue = selectedOptions[vo.option_id ?? ""] {
                    if vo.value == selectedValue {
                        matchCount += 1
                    }
                }
            }

            // If all selected options match this variant
            if matchCount == selectedOptions.count {
                return variant
            }
        }
    ...
    func didWishlishedItem(objProduct: Products?, strType: String?) {
        ...
        if strType == "Add"{
            ...
            self.objStoreItemDetailViewModel.deleteItemFromWishList(strVariantID: self.objProduct.variants?[0].wishlist_item_id ?? "")
        }
        self.tableView.reloadData()
    }
    ...
    //get product list on basis of selected product
