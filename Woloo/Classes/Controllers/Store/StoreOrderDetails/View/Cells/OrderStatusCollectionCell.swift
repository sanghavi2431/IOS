...
class OrderStatusCollectionCell: UICollectionViewCell {
    ...
    func configureNewOrderStatusCollectionCell(indexPath: IndexPath, objOrder: Orders?){
        self.objOrder = objOrder ?? Orders()
       
        self.currIndexPath = indexPath
        
        print("self.objOrderListings.fulfillment_status", self.objOrder.fulfillment_status ?? "")
        ...
        case 1:
            self.lblOrderPlaced.text = "Order Acepted"
            if self.objOrder.fulfillment_status == "fulfilled" || self.objOrder.fulfillment_status == "shipped" || self.objOrder.fulfillment_status == "delivered"{
                self.imgView.image = UIImage(named: "icon_check")
            }
            else{
                self.imgView.image = UIImage(named: "icon_uncheck")
        ...
        case 2:
            self.lblOrderPlaced.text = "Order Shipped"
           // self.imgView.image = UIImage(named: "icon_uncheck")
            if self.objOrder.fulfillment_status == "shipped" || self.objOrder.fulfillment_status == "delivered"{
                self.imgView.image = UIImage(named: "icon_check")
            }
            else{
                self.imgView.image = UIImage(named: "icon_uncheck")
        ...
        case 3:
            self.lblOrderPlaced.text = "Order Delivered"
            if self.objOrder.fulfillment_status == "delivered"{
                self.imgView.image = UIImage(named: "icon_check")
            }
            else{
                self.imgView.image = UIImage(named: "icon_uncheck")
