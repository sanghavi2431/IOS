...
//        objController.objAddress = self.objCustomerInfo.addresses ??  [StoreAddress]()
    ...
    func openProductListDetailView(objProduct: StoreProductCategories?) {
        ...
        objController.objStoreProductCategories = objProduct ?? StoreProductCategories()
        objController.listCategories = self.listFilterCategories
    ...
    //MARK: - StoreShowAllBrandVCDelegate
    ...
    func didRecieveemailpassResponse(response: UserRegisterWrapper) {
        ...
        self.objStoreHomePageViewModel.createWishListAPI()
    ...
    func didClickedBtnOrder() {
        ...
        objController.issComeFromHome = true
        self.navigationController?.pushViewController(objController, animated: true)
    }
    
    func didClickedBtnFavourite() {
        let objController = StoreShowAllProductVC(nibName: "StoreShowAllProductVC", bundle: nil)
...
//        }
    ...
    func didReceievGetProductCategoriesAPISuccess(objResponse: ProductcategoryWrapper) {
        ...
        for objCategory in objResponse.categories ?? [] {
            if (Utility.isEmpty(objCategory.parent_category_id ?? "") ){
                ...
                self.listFilterCategories.append(objCategory)
            }
            else if objCategory.name?.lowercased().contains("new in store") == true {
                // Append or handle category here'
                self.listNewInStores.append(objCategory)
            }
            else{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0{
            ...
            cell?.delegate = self
            cell?.configureStoreNewBrandCell(listProducts: self.listNewInStores)
            cell?.selectionStyle = UITableViewCell.SelectionStyle.none
            return cell!
        }
