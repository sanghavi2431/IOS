...
struct StoreHomePageViewModel{
    ...
    func restockSubscriptionAPI(strPhone: String?, strVariant: String?){
        ShopHostAPI().restockSubscriptionsAPI(strPhone: strPhone ?? "", strVariant_id: strVariant ?? "") { objCommonWrapper in
            self.delegate?.didRecieveRestockSubscriptionsAPIResponse(objWrapper: objCommonWrapper)
        } failure: { error in
            self.delegate?.didRecieveRestockSubscriptionsError(error: error?.localizedDescription ?? "")
        }
