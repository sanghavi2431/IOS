...
extension RateAToiletVC: UITableViewDelegate, UITableViewDataSource, SearchWolooForRateCellProtocol, SearchLocationEnrouteDelegate, RateAToiletViewModelProtocol, AddReviewSubmitBtnCellProtocol, AddReviewSliderCellDelegate, AddReviewStarCellDelegate, AddReviewDescriptionCellDelegate, WolooAlertPopUpViewDelegate, SearchWolooViewControllerDelegate, RateAToiletPopUpDelegate{
    ...
    func closePopUp() {
        self.objRateAToiletViewModel.creditUserCoins(blogId: 0, coins: Int(UserDefaultsManager.fetchAppConfigData()?.rate_a_toilet ?? "0"), isGift: 0, strRemarks: "Rate A Toilet", strType: "Manual Credit", woloo_id: self.objSearchWoloo.id ?? 0, wolooCoins: 0)
    }
    ...
    func submitReviewAPIV2() {
        ...
        NetworkManager(data: data,headers: headers, url: nil, service: .submitReview, method: .post, isJSONRequest: true).executeQuery {(result: Result<BaseResponse<SubmitReviewModel>, Error>) in
            ...
            case .success(let response):
                ...
                Global.addNetcoreEvent(eventname: self.netCoreEvents.rateAToilet, param: [
                    "woloo_id": self.objSearchWoloo.id ?? 0])
