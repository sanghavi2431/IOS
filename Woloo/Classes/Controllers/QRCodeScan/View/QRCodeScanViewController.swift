...
extension QRCodeScanViewController: LBXScanViewControllerDelegate {
    func scanFinished(scanResult: LBXScanResult, error: String?) {
        ...
        Global.addNetcoreEvent(eventname: self.netcoreEvent.wAHCertificatePoint, param: ["result":scanResult.strScanned ?? ""])
...
//                    NotificationCenter.default.post(name: Notification.Name.deepLinking, object: nil, userInfo: ["wahcertificate": "\(code)"])
                ...
                else if cleanedUrl.contains("powderroom") {
                    UserDefaultsManager.storeDeepLinkType(value: "powderroom")
                    print("URL contains powderroom")
                    let code = cleanedUrl.split(separator: "/").last ?? "No code"
                    print("URL contains powderroom with code: \(code)")
                    UserDefaultsManager.storeWahCode(value: "\(code)")
...
extension QRCodeScanViewController: QRCodeScanViewModelDelegate {
    ...
    func didReceiveWahCertificateResponse(objResponse: BaseResponse<WahCertificate>) {
        ...
        NotificationCenter.default.post(name: Notification.Name.deepLinking, object: nil, userInfo: ["wahcertificate": objResponse.results.woloo?.id ?? 0])
        
        NotificationCenter.default.post(name: Notification.Name.deepLinking, object: nil, userInfo: ["powderroom": objResponse.results.woloo?.id ?? 0])
       self.navigationController?.popViewController(animated: true)
    }
