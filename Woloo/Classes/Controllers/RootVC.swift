    ...
    func forceUpdatePopUp() {
        DispatchQueue.main.async {
            if let view = UIApplication.shared.windows.first?.rootViewController?.view, let controller = UIApplication.shared.windows.first?.rootViewController {
                let alert = WolooAlert(frame: view.frame, cancelButtonText: "Update", title: nil, message: "\((UserDefaultsManager.fetchAppConfigData()?.APP_VERSION?.update_text)!)" ?? "Please update to latest version", image: #imageLiteral(resourceName: "LogoGray"), controller: controller)
