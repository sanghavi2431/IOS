...
class InviteFriendVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        print("refcode: ", UserDefaultsManager.fetchUserData()?.profile?.ref_code ?? "")
    ...
    NetworkManager(headers: headers, url: nil, service: .userProfile, method: .get, isJSONRequest: false).executeQuery { (result: Result<BaseResponse<UserProfileModel>, Error>) in
        ...
        case .success(let response):
            ...
            DispatchQueue.main.async {
                ...
                self.strUniqueCode = response.results.profile?.ref_code ?? ""
            }
...
extension InviteFriendVC {
    func callAPIForShortLink(code: String) {
        ...
        guard let userRefCode = UserDefaultsManager.fetchUserData()?.profile?.ref_code else {
            Global.hideIndicator()
            return
        }
        
        // Base deep link (use /share/ path)
        let deepLink = "https://woloo.in/share/\(userRefCode)?referral_code=\(code)"
        
        // Construct long dynamic link (Firebase domain + iOS/Android params)
        let longDynamicLink = "https://woloo.page.link/?link=\(deepLink)&apn=in.woloo.www&ibi=in.woloo.app&isi=1571476207"
        ...
        guard let url = URL(string: "https://firebasedynamiclinks.googleapis.com/v1/shortLinks?key=AIzaSyDyJDAP9AhZDNDvFxB82N816xjWG9Lmji0") else {
            Global.hideIndicator()
            return
        }
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            guard (200...299).contains(httpResponse.statusCode), let data = data else {
                // Print Firebase error response
                if let data = data, let errorResponse = String(data: data, encoding: .utf8) {
                    print("❌ Firebase error response: \(errorResponse)")
                } else {
...
//UserDefaultsManager.fetchAppConfigData()?.URLS?.app_share_url
    func createReferralCodeLink(code: String) { // URL(string: "https://firebasedynamiclinks.googleapis.com/v1/shortLinks?key=AIzaSyAnqhrBVa89ey6O8b6xGvlx2UOmD6Np6Go")
        guard let link = URL(string: "\(UserDefaultsManager.fetchUserData()?.profile?.ref_code ?? "")/?referral_code=\(code)") else { return }
        let dynamicLinksDomainURIPrefix = UserDefaultsManager.fetchAppConfigData()?.URLS?.app_share_url ?? ""
