...
class TabBarController: UITabBarController {
    ...
    override func viewDidLoad() {
        ...
        if #available(iOS 26.0, *) {
            // Custom `CustomTabBar` layout fights system minimize / floating tab bar behavior.
...
extension TabBarController: UITabBarControllerDelegate {
    ...
    func tabBarController(_ tabBarController: UITabBarController, didSelect viewController: UIViewController) {
            ...
            case 1:
                print("🗺️ Loo Locator tab selected")
                UserDefaultsManager.clearDeepLinkType()
                UserDefaultsManager.clearWahcode()
            case 2:
                print("👤 Profile tab selected")
                Global.addNetcoreEvent(eventname: self.netcoreEvents.menuClick, param: ["":""])
                UserDefaultsManager.clearDeepLinkType()
                UserDefaultsManager.clearWahcode()
            ...
            // Example: if you need to know previous tab
            if previousIndex != currentIndex {
                print("🔄 Changed tab from \(previousIndex) to \(currentIndex)")
            }
