...
//        fo`rmatter.timeZone = TimeZone.current
    ...
    public func toDateSubscription(format: String = "yyyy-MM-dd") -> Date? {
        let formatter = DateFormatter()
        formatter.dateFormat = format
        formatter.timeZone = TimeZone(abbreviation: "UTC") // keep consistent
...
extension UIViewController {
    func showToast(message: String) {
        ...
        DispatchQueue.main.async {
            // Get keyWindow safely for iOS 13+
            ...
            let expectedSize = toastLabel.sizeThatFits(CGSize(width: maxWidth - 2*horizontalPadding, height: .greatestFiniteMagnitude))

            // Frame bgView
            let bgWidth = expectedSize.width + 2*horizontalPadding
            let bgHeight = expectedSize.height + 2*verticalPadding
            let yPosition = UIScreen.main.bounds.height - keyWindow.safeAreaInsets.bottom - bgHeight - 80

            bgView.frame = CGRect(x: (UIScreen.main.bounds.width - bgWidth)/2,
                                  y: yPosition,
                                  width: bgWidth,
                                  height: bgHeight)
            bgView.layer.cornerRadius = bgHeight / 2

            toastLabel.frame = CGRect(x: horizontalPadding,
                                      y: verticalPadding/2,
...
extension UIImage {
    static func gifFromAsset(named name: String) -> UIImage? {
        guard let asset = NSDataAsset(name: name) else {
            print("GIF asset not found: \(name)")
            return nil
        }
        return gif(data: asset.data)
    }
    ...
    static func gif(data: Data) -> UIImage? {
        ...
        for i in 0..<count {
            ...
            // Duration per frame (simple fallback)
            let delay = 0.1
            duration += delay
        }
...
extension UITextField{
    ...
    static func fixedFontTextField() -> UITextField {
...
class FixedContentSizeWindow: UIWindow {
    override var traitCollection: UITraitCollection {
        // Force the entire app to use .medium regardless of user settings
        let fixed = UITraitCollection(preferredContentSizeCategory: .medium)
        return UITraitCollection(traitsFrom: [super.traitCollection, fixed])
    }
}
